$(function(){
	...
	$phoneNum.focus(function(){
		...
		$(this).parent().next().removeClass('login-check-wrong').addClass('login-check-import').html('请输入常用手机号，避免忘记');
		//当此部分得焦是密码部分不论正确与否都不提示
			...
			if(!(re2.test(str2))){//用户名正确但密码格式不对
				...
				$checkPW.parent().next().removeClass('login-check-import').addClass('login-check-wrong').html('两次密码输入不一致');
					...
					if($checkNum.val()!==$checkText.html()){
						...
						$.cookie('userlist',JSON.stringify(cookieObj),{expires : 7,path:'/'});
